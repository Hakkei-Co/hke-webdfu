...
export class WebDFU {
  ...
  getDfuseFirstWritableSegment: any;
  ...
  private async requestOut(bRequest: number, data?: BufferSource, wValue = 0): Promise<number> {
    ...
    try {
      ...
      console.log('%c request out result ⏰ ', 'background:#6e6e6e; color: #cdfdce;, ⚛︎ WebDFU ⚛︎ requestOut ⚛︎ result', result);
  ...
  async setDfuseFirstWritableSegment(): Promise<any> {
    if (!this.dfuseMemoryInfo || !this.dfuseMemoryInfo.segments) {
      throw new WebDFUError("No memory map information available");
    }
  ...
  async getDfuseMaxReadSize(startAddr: number): Promise<any> {
    if (!this.dfuseMemoryInfo || !this.dfuseMemoryInfo.segments) {
      throw new WebDFUError("No memory map information available");
    }
  ...
  private async dfuseCommand(command: number, param = 0x00, len = 1) {
    ...
    const status = await this.poll_until((state) => state != dfuCommands.dfuDNBUSY);
